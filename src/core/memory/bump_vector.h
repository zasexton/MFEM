...
public:
    ...
    bump_vector(bump_vector&& other) noexcept : alloc_(other.alloc_) { move_from(std::move(other)); }
...
private:
    ...
    void reallocate(size_type new_cap) {
        ...
        // move-construct
        for (size_type i = 0; i < size_; ++i) ::new (static_cast<void*>(new_mem + i)) T(std::move_if_noexcept(data_[i]));
        destroy_all();
        deallocate();
