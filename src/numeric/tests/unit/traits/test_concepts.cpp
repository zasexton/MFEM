...
struct MockVector {
    ...
    MockVector operator*(T scalar) const {
        MockVector result = *this;
        for (auto& val : result.data) val *= scalar;
        return result;
    }
...
// Mock matrix type for testing matrix concepts
template<typename T>
struct MockMatrix {
    using value_type = T;
    ...
    MockMatrix(size_t r, size_t c) : m_rows(r), m_cols(c), data(r, std::vector<T>(c)) {}
    ...
    Vec solve(const Vec& b) const { return b; }
...
TEST(ConceptsTest, ArithmeticConcept) {
    ...
    EXPECT_TRUE(Arithmetic<std::complex<double>>);
...
TEST(ConceptsTest, ComplexFieldConcept) {
    EXPECT_TRUE(ComplexField<std::complex<double>>);
    EXPECT_TRUE(ComplexField<std::complex<float>>);
...
TEST(ConceptsTest, SolvableConcepts) {
    ...
    EXPECT_FALSE(Solvable<MockVector<double>>);
    EXPECT_FALSE(Solvable<double>);
}
...
template<typename T>
    requires Field<T> && !ComplexField<T>
T reciprocal(T x) {
    return T{1} / x;
}
