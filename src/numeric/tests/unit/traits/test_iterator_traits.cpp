...
public:
    ...
    TestStridedIterator(T* ptr, std::ptrdiff_t stride) : ptr_(ptr), stride_(stride) {}

    T& operator*() { return *ptr_; }
    TestStridedIterator& operator++() { ptr_ += stride_; return *this; }
    TestStridedIterator operator++(int) { auto tmp = *this; ++(*this); return tmp; }
    TestStridedIterator& operator--() { ptr_ -= stride_; return *this; }
    TestStridedIterator operator--(int) { auto tmp = *this; --(*this); return tmp; }

    TestStridedIterator& operator+=(difference_type n) { ptr_ += n * stride_; return *this; }
    TestStridedIterator& operator-=(difference_type n) { ptr_ -= n * stride_; return *this; }
    TestStridedIterator operator+(difference_type n) const { return TestStridedIterator(ptr_ + n * stride_, stride_); }
    TestStridedIterator operator-(difference_type n) const { return TestStridedIterator(ptr_ - n * stride_, stride_); }
    ...
    bool operator==(const TestStridedIterator& other) const { return ptr_ == other.ptr_; }
    bool operator!=(const TestStridedIterator& other) const { return !(*this == other); }
    bool operator<(const TestStridedIterator& other) const { return ptr_ < other.ptr_; }
    bool operator>(const TestStridedIterator& other) const { return other < *this; }
    bool operator<=(const TestStridedIterator& other) const { return !(other < *this); }
    bool operator>=(const TestStridedIterator& other) const { return !(*this < other); }
...
TEST_F(IteratorTraitsTest, BasicIteratorTraits) {
    ...
    EXPECT_TRUE((std::is_same_v<traits::iterator_category, std::random_access_iterator_tag>));
}
...
TEST_F(IteratorTraitsTest, NumericIteratorCategory) {
    ...
    MultiDimIter mdim_iter(vec.data(), strides, shape);  // Note: constructor order is (ptr, strides, shape)
...
TEST_F(IteratorTraitsTest, DistanceTraits) {
    // Random access - O(1)
    auto vec_dist = distance_traits<std::vector<int>::iterator>::compute(vec.begin(), vec.end());
    EXPECT_EQ(vec_dist, 5);
    EXPECT_TRUE(distance_traits<std::vector<int>::iterator>::is_constant_time);

    // Non-random access - O(n)
    auto lst_dist = distance_traits<std::list<int>::iterator>::compute(lst.begin(), lst.end());
...
TEST_F(IteratorTraitsTest, IteratorPairTraits) {
    using VecListPair = iterator_pair_traits<std::vector<int>::iterator, std::list<int>::iterator>;
    ...
    using VecVecPair = iterator_pair_traits<std::vector<int>::iterator, std::vector<int>::iterator>;
    ...
    using IntFloatPair = iterator_pair_traits<std::vector<int>::iterator, std::vector<float>::iterator>;
...
TEST_F(IteratorTraitsTest, IterationOptimization) {
    ...
    EXPECT_EQ(VecOpt::recommended_strategy(50000),
...
struct MockZipIterator {
    ...
    template<size_t I>
    auto get() {
        if constexpr (I == 0) return std::get<0>(iterators_);
        else return std::get<1>(iterators_);
    }
private:
...
TEST_F(IteratorTraitsTest, IntegrationWithBaseIterators) {
    ...
    using CheckedProps = iterator_properties<CheckedIterator<std::vector<int>::iterator>>;
    ...
    MultiDimIterator<int, 3> mdim_iter(vec.data(), strides, shape);  // Changed from int* to int
...
TEST_F(IteratorTraitsTest, EdgeCases) {
    ...
    EXPECT_TRUE(EmptyRangeVal::is_valid_range(empty_vec.begin(), empty_vec.end()));
    ...
    EXPECT_FALSE(RevVecProps::is_contiguous);  // Reverse iterators are not contiguous
}