...
} && (is_complex_v<T> || requires(T a, T b) {
                       { a < b } -> std::convertible_to<bool>;
                       { a > b } -> std::convertible_to<bool>;
                     });
...
public:
  ...
  Shape unsqueeze(int axis) const {
    ...
    new_dims.insert(new_dims.begin() + static_cast<std::ptrdiff_t>(norm_axis),
                    1);
    return Shape(new_dims);
  }

  // Flatten to 1D
  Shape flatten() const { return Shape{size()}; }
  ...
  Shape permute(const std::vector<size_t> &axes) const {
    if (axes.size() != dims_.size()) {
      throw std::invalid_argument(
          "Permutation must have same length as dimensions");
    }
    ...
    for (size_t i = 0; i < new_shape.size(); ++i) {
      if (static_cast<int>(i) == infer_idx) {
        ...
        result_dims.push_back(
            static_cast<size_t>(new_shape[i])); // Explicit cast
      }
    }
  ...
  bool is_broadcastable_with(const Shape &other) const {
    ...
    for (size_t i = 0; i < min_dims; ++i) {
      ...
      if (dims_[this_idx] != other.dims_[other_idx] && dims_[this_idx] != 1 &&
 ...
 */
template <typename Derived> class NumericBase {
public:
   ...
   */
  const Shape &shape() const noexcept { return derived().shape(); }
   ...
   */
  Layout layout() const noexcept { return derived().layout(); }

  /**
   * @brief Get device location
   */
  Device device() const noexcept { return derived().device(); }

  /**
   * @brief Check if data is contiguous in memory
   */
  bool is_contiguous() const noexcept { return derived().is_contiguous(); }
   ...
   */
  const std::type_info &dtype() const noexcept { return derived().dtype(); }

  /**
   * @brief Memory usage in bytes
   */
  size_t nbytes() const noexcept { return derived().nbytes(); }

  /*
   *  @brief Get number of dimensions
   */
  size_t ndim() const noexcept { return shape().rank(); }
  /**
   * @brief Create a copy
   */
  Derived copy() const { return derived().copy(); }

  /**
   * @brief Create a view (non-owning reference)
   */
  auto view() { return derived().view(); }

  auto view() const { return derived().view(); }

  /**
   * @brief Reshape to new dimensions
   */
  auto reshape(const Shape &new_shape) { return derived().reshape(new_shape); }

  /**
   * @brief Transpose (reverse dimensions)
   */
  auto transpose() const { return derived().transpose(); }

  /**
   * @brief String representation
   */
  std::string to_string() const { return derived().to_string(); }
...
public:
  template <typename T> static constexpr bool is_compliant() {
  ...
  template <typename T> static bool is_nan(T value) {
    if constexpr (std::is_floating_point_v<T>) {
      ...
      if (std::isnan(value.value()))
        return true;
      for (size_t i = 0; i < T::num_derivatives; ++i) {
        if (std::isnan(value.derivative(i)))
          return true;
      }
      return false;
    }
  ...
  template <typename T> static bool is_inf(T value) {
    if constexpr (std::is_floating_point_v<T>) {
      ...
      if (std::isinf(value.value()))
        return true;
      for (size_t i = 0; i < T::num_derivatives; ++i) {
        if (std::isinf(value.derivative(i)))
          return true;
      }
      return false;
    }
  ...
  template <typename T> static T quiet_nan() {
    if constexpr (std::is_floating_point_v<T>) {
 ...
 */
template <typename T = double> class NumericMetadata {
public:
  NumericMetadata()
      : is_floating_point_(std::is_floating_point_v<T>),
        is_integer_(std::is_integral_v<T>), is_complex_(is_complex_v<T>),
        is_ieee_compliant_(std::numeric_limits<T>::is_iec559) {};

  NumericMetadata(const std::type_info &dtype, const Shape &shape,
                  Layout layout = Layout::RowMajor, Device device = Device::CPU)
...
public:
  explicit ConvergenceError(const std::string &msg) : NumericError(msg) {}
};
