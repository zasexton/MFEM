 ...
 */
template <typename Iterator> struct basic_iterator_traits {
...
inline constexpr bool
    is_checked_iterator_v<fem::numeric::CheckedIterator<BaseIt>> = true;

template <typename T, size_t Rank>
inline constexpr bool
    is_multidim_iterator_v<fem::numeric::MultiDimIterator<T, Rank>> = true;

/**
 * @brief Detect iterator category with numeric extensions
 * Builds upon numeric_iterator_traits from iterator_base.h
 */
template <typename Iterator> struct numeric_iterator_category {
  using std_category =
  ...
  static constexpr NumericIteratorCategory value = [] {
    ...
    // Check for contiguous iterator (C++20)
                       ...
                       std::is_base_of_v<std::contiguous_iterator_tag,
                                         std_category>) {
      return NumericIteratorCategory::Contiguous;
    }
    // Standard categories
    else if constexpr (std::is_base_of_v<std::random_access_iterator_tag,
                                         std_category>) {
    ...
    } else if constexpr (std::is_base_of_v<std::forward_iterator_tag,
                                           std_category>) {
    ...
    } else if constexpr (std::is_base_of_v<std::output_iterator_tag,
                                           std_category>) {
 ...
 */
template <typename Iterator> struct is_random_access_iterator {
  static constexpr bool value = numeric_iterator_category_v<Iterator> >=
 ...
 */
template <typename Iterator> struct is_contiguous_iterator {
public:
  static constexpr bool value = [] {
    ...
    if constexpr (numeric_iterator_category_v<Iterator> ==
 ...
 */
template <typename Iterator> struct is_strided_iterator {
  static constexpr bool value = numeric_iterator_traits<Iterator>::is_strided ||
...
template <typename Iterator>
inline constexpr bool is_strided_iterator_v =
    is_strided_iterator<Iterator>::value;

/**
 * @brief Get stride of iterator (if applicable)
 */
template <typename Iterator> struct iterator_stride {
 ...
 */
template <typename Iterator> struct is_parallel_safe_iterator {
  static constexpr bool value =
...
template <typename Iterator>
inline constexpr bool is_parallel_safe_iterator_v =
    is_parallel_safe_iterator<Iterator>::value;

/**
 * @brief Iterator properties aggregator
 * Enhanced to work with base iterator types
 */
template <typename Iterator> struct iterator_properties {
  ...
  static constexpr NumericIteratorCategory category =
      numeric_iterator_category_v<Iterator>;
  ...
  static constexpr bool is_parallel_safe =
      is_parallel_safe_iterator_v<Iterator>;

  // From base traits
  static constexpr bool is_numeric = base_numeric_traits::is_numeric;
  static constexpr bool is_ieee_compliant =
      base_numeric_traits::is_ieee_compliant;
  ...
  static constexpr bool supports_simd =
      is_contiguous && std::is_arithmetic_v<value_type> &&
 ...
 */
template <typename Iterator> struct distance_traits {
  ...
  static constexpr bool is_constant_time =
      is_random_access_iterator_v<Iterator>;
};
 ...
 */
template <typename Iterator> struct advance_traits {
  using difference_type =
      typename std::iterator_traits<Iterator>::difference_type;
 ...
 */
template <typename Iterator> struct range_validation {
  ...
  static bool is_empty(Iterator first, Iterator last) { return first == last; }
 ...
 */
template <typename Iterator1, typename Iterator2> struct iterator_pair_traits {
  ...
  static constexpr bool both_contiguous = is_contiguous_iterator_v<Iterator1> &&
                                          is_contiguous_iterator_v<Iterator2>;

  static constexpr bool can_parallel_process =
      both_random_access && is_parallel_safe_iterator_v<Iterator1> &&
 ...
 */
template <typename Iterator> struct simd_iteration_traits {
  using value_type = typename std::iterator_traits<Iterator>::value_type;

  static constexpr bool can_vectorize = is_contiguous_iterator_v<Iterator> &&
  ...
  static bool is_aligned(const Iterator &it) {
    if constexpr (std::is_pointer_v<Iterator>) {
      return reinterpret_cast<uintptr_t>(&(*it)) %
                 (vector_width * sizeof(value_type)) ==
             0;
    } else {
 ...
 */
template <typename Iterator> struct multidim_iterator_traits {
  static constexpr bool is_multidim = is_multidim_iterator_v<Iterator>;
  ...
  template <typename It> static auto get_indices(const It &it) {
  ...
  static constexpr bool supports_broadcasting =
      false; // Base MultiDimIterator doesn't support broadcasting
};

/**
 * @brief Iteration optimization hints
 */
template <typename Iterator> struct iteration_optimization {
  ...
  // TODO: should the cache_line and l1_cache sizes be fetched from the machine
  // hardware info instead of
  //       being hard coded?
 ...
 */
template <typename Iterator> struct checked_iterator_traits {
  static constexpr bool is_checked = is_checked_iterator_v<Iterator>;
  ...
  using base_iterator_t =
      std::conditional_t<is_checked_iterator_v<It>,
                         typename It::iterator, // CheckedIterator has
                                                // 'iterator' typedef
                         It>;
 ...
 */
template <typename Iterator> struct is_zip_iterator {
  static constexpr bool value = requires(Iterator it) {
    typename Iterator::iterator_tuple; // Tuple of iterators
    {
      it.template get<0>()
    }; // Access individual iterators - fixed template keyword
  };
};
 ...
 */
template <typename Iterator> struct is_transform_iterator {
  static constexpr bool value = requires(Iterator it) {
...
template <typename Iterator>
inline constexpr bool is_transform_iterator_v =
