...
private:
    ...
    difference_type row_stride_value() const noexcept {
        if constexpr (is_row_major) {
            return static_cast<difference_type>(cols_);
        } else {
            return difference_type{1};
        }
    }

    difference_type col_stride_value() const noexcept {
        if constexpr (is_row_major) {
            return difference_type{1};
        } else {
            return static_cast<difference_type>(rows_);
        }
    }
...
public:
    ...
    /**
     * @brief Lightweight view covering the entire matrix
     */
    submatrix_view_type view();
    const_submatrix_view_type view() const;

    /**
     * @brief Submatrix view defined by half-open index ranges
     */
    submatrix_view_type submatrix(size_type row_start, size_type row_end,
                                  size_type col_start, size_type col_end);
    const_submatrix_view_type submatrix(size_type row_start, size_type row_end,
                                        size_type col_start, size_type col_end) const;

    /**
     * @brief NumPy-like slicing interface using Slice descriptors
     */
    submatrix_view_type operator()(const Slice& row_slice, const Slice& col_slice);
    const_submatrix_view_type operator()(const Slice& row_slice, const Slice& col_slice) const;
     ...
     */
    transpose_view_type T_();
    const_transpose_view_type T_() const;
...
/**
 * @brief Lightweight 2D view over matrix data
 */
template<typename T>
...
/**
 * @brief Transposed matrix view implemented lazily
 */
template<typename T>
...
template<typename T>
MatrixTransposeView<T> MatrixView<T>::transpose() const {
    return MatrixTransposeView<T>(*this);
}

// === Matrix member function definitions requiring view types ===

template<typename T, typename Storage, StorageOrder Order>
typename Matrix<T, Storage, Order>::transpose_view_type
Matrix<T, Storage, Order>::transpose() {
    return transpose_view_type(data(), cols_, rows_, col_stride_value(), row_stride_value());
}

template<typename T, typename Storage, StorageOrder Order>
typename Matrix<T, Storage, Order>::const_transpose_view_type
Matrix<T, Storage, Order>::transpose() const {
    return const_transpose_view_type(data(), cols_, rows_, col_stride_value(), row_stride_value());
}

template<typename T, typename Storage, StorageOrder Order>
typename Matrix<T, Storage, Order>::transpose_view_type
Matrix<T, Storage, Order>::T_() {
    return transpose();
}

template<typename T, typename Storage, StorageOrder Order>
typename Matrix<T, Storage, Order>::const_transpose_view_type
Matrix<T, Storage, Order>::T_() const {
    return transpose();
}

template<typename T, typename Storage, StorageOrder Order>
typename Matrix<T, Storage, Order>::submatrix_view_type
Matrix<T, Storage, Order>::view() {
    return submatrix_view_type(data(), rows_, cols_, row_stride_value(), col_stride_value());
}

template<typename T, typename Storage, StorageOrder Order>
typename Matrix<T, Storage, Order>::const_submatrix_view_type
Matrix<T, Storage, Order>::view() const {
    return const_submatrix_view_type(data(), rows_, cols_, row_stride_value(), col_stride_value());
}

template<typename T, typename Storage, StorageOrder Order>
typename Matrix<T, Storage, Order>::submatrix_view_type
Matrix<T, Storage, Order>::submatrix(size_type row_start, size_type row_end,
                                     size_type col_start, size_type col_end) {
    if (row_start > row_end || col_start > col_end ||
        row_end > rows_ || col_end > cols_) {
    ...
    const size_type col_count = col_end - col_start;
    if (row_count == 0 || col_count == 0) {
        return submatrix_view_type(data(), 0, 0, row_stride_value(), col_stride_value());
    }
    auto base_ptr = data() + linear_index(row_start, col_start);
    return submatrix_view_type(base_ptr, row_count, col_count,
                               row_stride_value(), col_stride_value());
}
...
Matrix<T, Storage, Order>::submatrix(size_type row_start, size_type row_end,
    ...
    const size_type col_count = col_end - col_start;
    if (row_count == 0 || col_count == 0) {
        return const_submatrix_view_type(data(), 0, 0, row_stride_value(), col_stride_value());
    }
    auto base_ptr = data() + linear_index(row_start, col_start);
    return const_submatrix_view_type(base_ptr, row_count, col_count,
                                     row_stride_value(), col_stride_value());
}

template<typename T, typename Storage, StorageOrder Order>
typename Matrix<T, Storage, Order>::submatrix_view_type
Matrix<T, Storage, Order>::operator()(const Slice& row_slice, const Slice& col_slice) {
    return view()(row_slice, col_slice);
}

template<typename T, typename Storage, StorageOrder Order>
typename Matrix<T, Storage, Order>::const_submatrix_view_type
Matrix<T, Storage, Order>::operator()(const Slice& row_slice, const Slice& col_slice) const {
    return view()(row_slice, col_slice);
}
